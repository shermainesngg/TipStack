...
function FeaturedIntegration({ content }: { content: Content }) {
  const snippet = extractCodeSnippet(content.body, 6);
    ...
    <Link href={getArticleUrl(content.tags_category, content.slug)} className="block group">
      <motion.article
        className="rounded-2xl bg-[#f8fcf9] p-8 lg:p-10
          ...
          dark:bg-[#1A241E] dark:shadow-[0_1px_3px_rgba(0,0,0,0.2),0_8px_24px_rgba(0,0,0,0.3)]"
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
          ...
          <div className={snippet ? "lg:w-1/2" : ""}>
            <div className="flex items-center gap-2 mb-3">
              <Blocks className="size-3.5 text-[#2D6040]" />
...
function IntegrationCard({ content }: { content: Content }) {
  const topTags = content.tags_tool
    .slice(0, 2)
    .map((t) => ({ label: t, category: "tool" as const }));
  ...
  return (
    <Link href={getArticleUrl(content.tags_category, content.slug)} className="block group">
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          <Blocks className="size-3 text-[#2D6040]" />
...
export function McpIntegrationsLayout({ content }: { content: Content[] }) {
  ...
  return (
    <motion.div
      ...
      animate="visible"
    >
      <FeaturedIntegration content={featured} />
      ...
      {rest.length > 0 && (
        ...
        >
          {rest.map((item) => (
            <IntegrationCard key={item.id} content={item} />
          ))}
        </motion.div>
      )}
    </motion.div>
