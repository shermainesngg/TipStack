...
function FeaturedFeature({ content }: { content: Content }) {
  ...
  return (
    <Link href={`/content/${content.slug}`} className="block group">
      <motion.article
        className="rounded-2xl bg-[#fdfaf5] p-8 lg:p-10
          ...
          dark:bg-[#241F18] dark:shadow-[0_1px_3px_rgba(0,0,0,0.2),0_8px_24px_rgba(0,0,0,0.3)]"
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
          ...
          <div className={snippet ? "lg:w-1/2" : ""}>
            <div className="flex items-center gap-2 mb-3">
              <Terminal className="size-3.5 text-[#9B6B2F]" />
              <span className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#9B6B2F]">
                Featured
              </span>
            </div>
...
function FeatureCard({ content }: { content: Content }) {
...
export function ClaudeCodeFeaturesLayout({ content }: { content: Content[] }) {
  ...
  return (
    <motion.div
      ...
      animate="visible"
    >
      <FeaturedFeature content={featured} />
      ...
      {rest.length > 0 && (
        ...
        >
          {rest.map((item) => (
            <FeatureCard key={item.id} content={item} />
          ))}
        </motion.div>
      )}
    </motion.div>
