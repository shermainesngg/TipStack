...
function PipelineCard({ content }: { content: Content }) {
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
                ...
                {steps.map((step, i) => (
                  <div key={i} className="flex items-start gap-3 py-2">
                    <div className="flex flex-col items-center shrink-0">
                      <div className="size-6 rounded-full bg-[#f3eff8] dark:bg-[#2A1F3D] flex items-center justify-center text-[11px] font-semibold text-[#5E3F96] dark:text-[#B89DD4]">
                        {i + 1}
                      </div>
                      {i < steps.length - 1 && (
                        <div className="w-px h-4 bg-[#dde4db] dark:bg-[#3A433A] mt-1" />
                      )}
                    </div>
          ...
          <div className={steps.length > 0 ? "lg:w-3/5" : ""}>
            <div className="flex items-center gap-2 mb-3">
              <Workflow className="size-3.5 text-[#5E3F96]" />
...
function StepCard({ content }: { content: Content }) {
  ...
  return (
    <Link href={`/content/${content.slug}`} className="block group">
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          <Workflow className="size-3 text-[#5E3F96]" />
          {stepCount > 0 && (
            <span className="text-[11px] font-medium text-[#5E3F96] dark:text-[#B89DD4]">
              {stepCount} steps
            </span>
          )}
        </div>
...
export function WorkflowAutomationLayout({
  content,
}: {
  content: Content[];
}) {
  ...
  return (
    ...
    >
      ...
      {rest.length > 0 && (
        ...
        >
          {rest.map((item) => (
            <StepCard key={item.id} content={item} />
          ))}
        </motion.div>
      )}
    </motion.div>
