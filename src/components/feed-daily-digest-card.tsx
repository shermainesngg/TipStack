...
const OG_BASE = "/api/og/feed-daily";

export function FeedDailyDigestCard({ date }: { date?: string }) {
  ...
  async function handleDownload(format: "og" | "social") {
    ...
    a.download = `tipstack-feed-daily-${format}${date ? `-${date}` : ""}.png`;
    a.click();
  ...
  return (
    <div className="mb-4">
      <div className="rounded-xl overflow-hidden border border-[#dde4db] dark:border-[#2A322A] shadow-sm max-w-[540px]">
        ...
        <img
          src={displayUrl}
          alt="TipStack Daily — AI workflow tips summary"
          loading="lazy"
